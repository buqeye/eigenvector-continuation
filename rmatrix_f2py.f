! Wrapper for the rmatrix code. 
! pot_output_rmatrix() returns scattering phase shifts and 
...
c blo1, blo2=Bloch operator at the limits of the next intervals (nu=0)
      ...
      tc(i1,i2,1)=xx/dsqrt(xi2*xj2)
      blo0(i1,i2)=1/dsqrt(xi2*xj2)
      else
      yy=dsqrt(xj2/xi2**3)*(2*xi*xj+3*xi-xj-4*xi**2)/(xj-xi)**2
      xlb=dsqrt(xi*xj/(1-xi)/(1-xj))*(nr*(nr+1)-1/(1-xj))
      xla=dsqrt((1-xi)*(1-xj)/xi/xj)*(-nr*(nr+1)+1/xj)
      tc(i1,i2,is)=yy+xlb-xla
      blo1(i1,i2)=dsqrt((1-xi)*(1-xj)/xi/xj)
      blo2(i1,i2)=dsqrt(xi*xj/(1-xi)/(1-xj))
      end if
   ...
   30 continue
      if(is.eq.1)then
      q2(:,1)=1/dsqrt(xle(:)*(1-xle(:)))
      q1(:,1)=0
      else
      q2(:,is)=dsqrt(xle(:)/(1-xle(:)))
      q1(:,is)=-1/q2(:,is)
      end if
   ...
   31 continue
     ...
     & necm,ecm_mesh,nr,rmax,r_mesh,pot_mesh, lpot_mesh,
     & rmax0,wle,xle,tc,blo0,blo1,blo2,q1,q2, 
     & angL,delta,wf_in_re,wf_in_im)
...
! should have ndim length    
       ...
       double precision ecm_mesh(necm),r_mesh(nr),pot_mesh(nr,nr),
     &  lpot_mesh(nr)
       double precision delta(necm), wf_in_re(necm,nr),wf_in_im(necm,nr)
     ...
     & cf(nr*ns,nwf2,nc), cpnl((nr*ns)**2,ndim,ndim)
       ...
       double precision eta(nch),qk(nch),zrma(nr*ns),kc,ecm,tp
...
Cf2py  intent(in) pot_mesh
Cf2py  depend(nr) pot_mesh 
Cf2py  intent(in) lpot_mesh
Cf2py  depend(nr) lpot_mesh
...
Cf2py  depend(nr) wf_in_im
      ...
      do i=1,ncp1
      r=r_mesh(i)
      cpot(i,1,1)=2*kc/r + lpot_mesh(i)*(2*mR/hbarc**2) ! the local coulomb pot x 2 mR, in fm^-2
...
! becomes non-local because of regularization. 
      ...
      do i=1, ncp1 
         do ii=1, ncp1 
         j=j+1 
         cpnl(j,1,1)= (2*mR/hbarc**2)*pot_mesh(i,ii)
         end do 
      end do 
! note pot_mesh in MeV units, while cpnl in fm^-2, this is for nonlocal pot. 
     ...
     & rmax0,wle,xle,tc,blo0,blo1,blo2,q1,q2)
        tp=datan2(aimag(cu(1,1)),real(cu(1,1)))/2 
        delta(i)=tp
        ctp=dcmplx(dsin(tp) , dcos(tp))*cf(:,1,1)/2
        wf_in_re(i,:)= real(ctp)/dcos(tp)/qk(1)
        wf_in_im(i,:)=aimag(ctp)/dcos(tp)/qk(1)   
...
c Calculation of matrix C
    ...
    8 continue
     ...
     1 ch(m1+1:m1+nr,m1+1:m1+nr,is)-xc(i1,1)*(blo2/rmax2-blo1/rmax1)
      ...
      do irp=1,nr
      ii=ii+1
      ch(m1+ir,m2+irp,is)=ch(m1+ir,m2+irp,is)+dsqrt(wle(ir)*wle(irp))*
...
c Calculation of matrices Z_I and Z_O
      ...
      do 20 m2=1,nopen
      i2=npo(m2,1)
      crma2(m1,m2,1:ns)=crma0(i1,i2,1:ns)*dsqrt(qk(i1)/qk(i2))
   20 continue
      do 22 m1=1,nopen
...
c Loop over the entrance channels
      ...
      do 101 m1=1,nopen
      i1=npo(m1,1)
      cfp(i1,ns)=-cu(m1,nvex)*cop(m1)*dsqrt(qk(i2)/qk(i1))
...
c Values of the wave functions at the mesh points
      ...
      mm=0
      do 123 is=1,ns
      cf(mm+1:mm+nr,i1,ic)=cf(mm+1:mm+nr,i1,ic)/dsqrt(rmax0*wle(1:nr))
      mm=mm+nr
  123 continue
  122 continue
  100 continue
...
c cwftab(npoin): complex vales of the wave function at r=h, 2*h, 3*h, ... 
    ...
    2 continue
      ...
      co=dcmplx(gc(ll+1),fc(ll+1))*dsqrt(qk(nom)/qk(iv))
      cfx=-cu(nop,nom)*co
