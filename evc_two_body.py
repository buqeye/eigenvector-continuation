...
from two_body_pot import two_body_pot
...
class EigenvectorContinuationScattering:
    """
    Set up and carry out eigenvectorcontinuation for the two_body_pot objects. 
      for scattering instead of bound states.
...
# otherwise, EVC looses the point. 
        ...
        delta_U = np.zeros( (num_terms, num_terms) )
            ...
            for i, pc_i in enumerate(self.pc_array):
                for j, pc_j in enumerate(self.pc_array):
                    delta_U[i,j] =  (2*mu/hbar**2) \
                        * nonlocal_matrix_element_gauss( pc_i.scatt_wf_in_re_mesh[iE], 
                                                  pc_j.scatt_wf_in_re_mesh[iE], 
